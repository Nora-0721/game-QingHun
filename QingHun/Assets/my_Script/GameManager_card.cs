...
public class GameManager : MonoBehaviour
{
    ...
    public Sprite qingSprite;   // 小青的图案
    public Sprite xianSprite;   // 许仙的图案
    public Sprite baiSprite;    // 小白的图案
    ...
    private IEnumerator MoveCardsToCenterAndDamage(int damage)
    {
        ...
        {
            // 修改后
            if (!isDialogueEnded && (dialogueList[currentDialogueIndex].speaker == "小青"|| dialogueList[currentDialogueIndex].speaker == "许仙"|| dialogueList[currentDialogueIndex].speaker == "小白"))
            {
    ...
    private IEnumerator BossAttackAtStart()
    {
        ...
        if (currentDialogueIndex < dialogueList.Count)
        {
            ...
            // 修改后
            if (!isDialogueEnded && dialogueList[currentDialogueIndex].speaker == "法海")
            {
                yield return StartCoroutine(ShowDialogue(currentDialogueIndex));
    ...
    private IEnumerator ShowDialogue(int index)
    {
        ...
        {
            case "小青":
                spiritImage.sprite = qingSprite;
                break;
            case "许仙":
                spiritImage.sprite = xianSprite;
                break;
            case "小白":
                spiritImage.sprite = baiSprite;
                break;
            default:
                // 其他角色保持原图或隐藏
    ...
    private void UpdateUI()
    {
        ...
        //    Debug.Log($"Spirit按钮状态: {shouldShow}"); // 调试输出
        //}
        // 处理血条完全空的情况
        if (playerHealth <= 0)
        {
            ...
            if (SceneTransitionManager1.Instance != null)
            {
                SceneTransitionManager1.Instance.LoadSceneWithTransition("cuoshi9");
            }
            else
            {
